...
public class ControllerFreqAscendingly implements ActionListener {

	View view;
	Model model;
	
	public ControllerFreqAscendingly(View view, Model model) {
		this.view = view;
		this.model = model;
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (model.isInitialized()) {
			 ...
			 */
			if (model.isSimple()) {
				builder.append("Frequency Ascendingly");
			}
			else {
				builder.append("<h1 style='text-align: center; margin: 0px'> Frequency Ascendingly </h1>");
			}
			
			/*
			 * The body with all words
			 */
			for (int i = min; i <= map.getMax(); i++) {
